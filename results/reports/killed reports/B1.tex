...
\begin{enumerate}
  \item N3 (DVOL $z$-score $> \theta$, DVOL $\geq \delta$): OOS Sharpe $+2.95$,
        $1.2$ trades/week (\textbf{A1}).
  \item P3 (OI-Price Divergence, DD regime, DVOL $\geq \delta$): OOS Sharpe $+5.18$
...
\begin{itemize}
  ...
  \item \textbf{DVOL-filtered} ($\geq \delta$): ratio $> 1.0$ and bootstrap
        $p \leq 0.05$.
...
Calmar ratio & $13.58$ \\
\midrule
Combined (Q5c $\geq 0.5$ AND $N3z > \theta$, DVOL $\geq 52$) & $+4.99$ ($n = 64$ trades) \\
\bottomrule
\end{tabular}
\end{center}
...
The combined rule (Q5c $\geq 0.5$ AND $N3z > \theta$, DVOL $\geq 52$)
...
The DVOL $\geq \delta$ filter reduces the OOS sample from 863 to 333
