...
DVOL $\geq 54$ regime filter threshold is the only tuned parameter, and it
...
Sharpe peaks at $\theta = 0.75$ with Sharpe $= 2.37$, net PnL
...
\begin{table}[H]
\centering
\caption{Year-by-year long-short backtest ($\theta = 0.75$, maker cost, daily)}
\label{tab:yearly}
...
we evaluate the long-short rule ($\theta = 0.75$, maker cost) over the full
OOS 2024+ period ($n = 863$ days) at varying DVOL filters:

\begin{table}[H]
\centering
\caption{DVOL entry-filter sweep (OOS 2024+, $n=863$ days, $\theta=0.75$, maker cost)}
...
spot is DVOL $\geq 54$--57: Sharpe $2.20$--$3.46$ with the maximum
drawdown shrinking from $5{,}055\bp$ (unfiltered) to $1{,}624\bp$ at
DVOL $\geq 57$.

\paragraph{Robustness to the choice of threshold.}
The Sharpe at DVOL $\geq 54$ is $2.20$ and at DVOL $\geq 57$ is $3.46$. This
$1.26$ difference is not concentrated at a cliff: the Sharpe increases
smoothly from 46 to 60. The DVOL $= 54$ threshold is adopted for the
...
The frozen rule enters both long (N3$_t > 0.75$) and short (N3$_t < -0.75$)
positions conditional on DVOL $\geq 54$.  The fear-resolution mechanism
...
perceived risk subsides.  The short side (DVOL above 54 but below its
...
\midrule
LONG  ($N3_z > +0.75$) & 105 & $+6{,}080$ & $3.60$ & 52.4\% & $+274$ & $1{,}119$ \\
SHORT ($N3_z < -0.75$) &  38 & $+403$     & $0.81$ & 47.4\% & $+222$ & $1{,}453$ \\
\midrule
...
The DVOL $\geq 54$ threshold is chosen using OOS 2024+ data. While
Table~\ref{tab:dvol_sweep} shows a smooth relationship (not a cliff
at 54), there is residual data snooping in the threshold choice.
A fixed-rule strategy using DVOL $\geq 51$ (the historical median)
instead of $54$ produces Sharpe $1.68$ --- lower but still strongly
profitable.
